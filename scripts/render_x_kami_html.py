#!/usr/bin/env python3
"""Render Kami-style X HTML cards to PNG with Chrome."""
...
def render_card(html_path: Path, png_path: Path) -> None:
    with tempfile.TemporaryDirectory(prefix="x-kami-chrome-", dir="/private/tmp") as profile:
        command = [
            str(CHROME),
            "--headless",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Render Kami-style X HTML cards to PNG.")
...
def main() -> int:
    ...
    print(f"Rendered Kami-style X PNG cards: {card_dir}")
